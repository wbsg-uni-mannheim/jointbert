...
feature_combinations = [['title', 'specs']]

for feature_combination in feature_combinations:
    for learning_rate in lr_range:
        for file in glob.glob('../../../data/processed/di2kg/magellan/monitor/formatted/*'):
            if 'trainonly' in file and 'metadata' not in file and '_pairs_' in file:
                train_set = file
                valid_set = file.replace('trainonly','valid')
                test_set = valid_set
                pred_set = ['../../../data/processed/di2kg/magellan/monitor/formatted/monitor-gs_magellan_pairs_formatted.csv']
                ...
                run_deepmatcher.run_dm_model(train_set, valid_set, test_set, experiment_name, gpu_id, epochs, pos_neg_ratio,
                                             batch_size, lr, lr_decay, embedding, nn_type, comp_type, special_name,
                                             features,
