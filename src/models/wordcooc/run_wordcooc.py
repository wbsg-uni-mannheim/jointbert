...
if __name__ == '__main__':
    ...
    for file in glob.glob('../../../data/processed/di2kg/wordcooc/monitor/*'):
        if 'train_' in file and '-gs' not in file:
            valid = file.replace('train_', 'valid_')
    
            test = os.path.basename(file)
            test = test.replace('.pkl.gz', '_{}-gs.pkl.gz'.format(experiment_name))
            test = '../../../data/processed/di2kg/wordcooc/monitor/{}'.format(test)
    ...
    experiment_name = 'abt-buy'

    for file in glob.glob('../../../data/processed/abt-buy/wordcooc/abt-buy/*'):
        if 'train_' in file and '-gs' not in file:
            valid = file.replace('train_', 'valid_')

            test = os.path.basename(file)
            test = test.replace('.pkl.gz', '_{}-gs.pkl.gz'.format(experiment_name))
            test = '../../../data/processed/abt-buy/wordcooc/abt-buy/{}'.format(test)

            run_wordcooc(file, valid, test, feature_combinations, classifiers, experiment_name, dataset_name='abt-buy',
                         write_test_set_for_inspection=True)

    # dblp-scholar experiment
    feature_combinations = ['title', 'title+authors', 'title+authors+venue', 'title+authors+venue+year']
    experiment_name = 'dblp-scholar'

    for file in glob.glob('../../../data/processed/dblp-scholar/wordcooc/dblp-scholar/*'):
        if 'train_' in file and '-gs' not in file:
            valid = file.replace('train_', 'valid_')

            test = os.path.basename(file)
            test = test.replace('.pkl.gz', '_{}-gs.pkl.gz'.format(experiment_name))
            test = '../../../data/processed/dblp-scholar/wordcooc/dblp-scholar/{}'.format(test)
    ...
    experiment_name = 'company'

    for file in glob.glob('../../../data/processed/company/wordcooc/company/*'):
        if 'train_' in file and '-gs' not in file:
            valid = file.replace('train_', 'valid_')

            test = os.path.basename(file)
            test = test.replace('.pkl.gz', '_{}-gs.pkl.gz'.format(experiment_name))
            test = '../../../data/processed/company/wordcooc/company/{}'.format(test)
