...
if __name__ == '__main__':
    ...
    if BUILD_ABTBUY:
        ...
        feature_combinations = ['name', 'name+description', 'name+description+price']

        file = '../../../data/interim/abt-buy/abt-buy-train.json.gz'

        valid = file.replace('train', 'valid')
        valid = valid.replace('.json.gz', '.csv')

        test = file.replace('train', 'gs')

        preprocess_wordcooc(file, columns_to_preprocess, feature_combinations, experiment_name='abt-buy', dataset_name='abt-buy', valid_set=valid,
                            test_set=test)
    ...
    if BUILD_DI2KG:
        ...
        feature_combinations = ['title', 'title+specs']

        file = '../../../data/interim/di2kg/monitor-train.json.gz'
        ...
        preprocess_wordcooc(file, columns_to_preprocess, feature_combinations, experiment_name='monitor', dataset_name='di2kg', valid_set=valid,
                            test_set=test)

    if BUILD_DBLPSCHOLAR:
        columns_to_preprocess = ['title', 'authors', 'venue', 'year']
        feature_combinations = ['title', 'title+authors', 'title+authors+venue', 'title+authors+venue+year']

        file = '../../../data/interim/dblp-scholar/dblp-scholar-train.json.gz'
        ...
        preprocess_wordcooc(file, columns_to_preprocess, feature_combinations, experiment_name='dblp-scholar',
                            dataset_name='dblp-scholar', valid_set=valid,
                            test_set=test)

    if BUILD_COMPANY:
        columns_to_preprocess = ['content']
        feature_combinations = ['content']

        file = '../../../data/interim/company/company-train.json.gz'
        ...
        preprocess_wordcooc(file, columns_to_preprocess, feature_combinations, experiment_name='company',
                            dataset_name='company', valid_set=valid,
                            test_set=test)