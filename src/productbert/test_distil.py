...
def main(config):
    ...
    data_loader = getattr(module_data, config['data_loader']['type'])(
        ...
        num_workers=8,
        tokenizer_name=config['data_loader']['args']['tokenizer_name'],
    ...
    with torch.no_grad():
        for i, inputs in enumerate(tqdm(data_loader)):
            data, attn_mask, target = inputs['input_ids'].to(device), inputs['attention_mask'].to(device), inputs['labels'].to(device)
